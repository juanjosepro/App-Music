  ...
  <div>
     <v-row>
      ...
      <v-col sm="12" md="6" lg="8">
        <v-card raised>
          <v-card-title>
              <span>Category <em>{{musics.name}}</em> - <small>Select the music to play</small></span>
          </v-card-title>
          <v-card-text>
                  ...
                  <v-card raised>
                    <v-card-text>
                      <v-simple-table>
                        <template v-slot>
                          <tbody>
                            <tr v-for="music in musics.musics" :key="music.id" @click="play(music)"
...
export default {
  ...
  created(){
    ...
    this.getMusicByCategory(id)    
  },
  methods: {
    ...mapActions('music', ['getMusicByCategory']),
    // Methods Audio
    play (music) {
      if (typeof music.url != "undefined") {
        this.current = music;
      }else{
        this.current = this.musics.musics[this.index];
      }
      ...
      this.player.addEventListener('ended', function () {
        this.index++;
        if (this.index > this.musics.musics.length - 1) {
          this.index = 0;
        }
    ...
    prev () {
      this.index--;
      if (this.index < 0) {
        this.index = this.musics.musics.length - 1;
      }
